...
**Goal:** Export character sheet as a 3-page PDF faithful to the WOTC D&D 5e official layout.

**Architecture:** `@react-pdf/renderer` generates PDFs client-side. A `CharacterSheetDocument` React component renders 3 pages (combat/stats, spells, history/inventory). A `PdfExportButton` triggers generation via `pdf().toBlob()` and downloads the file. The PDF module is lazy-loaded to avoid bloating the main bundle (~1.5MB).

**Tech Stack:** @react-pdf/renderer, existing lib/dnd5e.ts calculations, data/skills.json for PT-BR names

**Spec:** `docs/superpowers/specs/modules/06-pdf-export.md`
...
|------|--------|----------------|
| `lib/pdfExport.tsx` | Create | PDF Document with 3 pages + styles + `generateCharacterPdf()` |
| `components/character/PdfExportButton.tsx` | Create | Button with lazy import + blob download |
| `app/character/[id]/page.tsx` | Modify | Add PdfExportButton to character sheet header |

---

### Task 1: Install @react-pdf/renderer and create PDF foundation
...
git commit -m "feat: add PDF export document with 3-page WOTC-style layout

Page 1: attributes, saving throws, skills, combat, attacks, conditions.
Page 2: spell slots, spell lists by level.
Page 3: traits, appearance, backstory, inventory, coins, notes.
Uses @react-pdf/renderer with EB Garamond font."
```

---

### Task 2: PdfExportButton and wire into character page
...
Then find the back button / header area at the top of the rendered JSX and add the PdfExportButton next to it. Look for the `<Button variant="ghost" onClick={() => router.push("/character")}` line and place the export button in a flex row with it:

Replace the back button section with:
...
Lazy-loads @react-pdf/renderer on click to keep main bundle small.
...
```bash
git add docs/
git commit -m "docs: mark PDF Export (T19) as complete in roadmap and spec"
```
