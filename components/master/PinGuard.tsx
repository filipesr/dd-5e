...
export function PinGuard({ children }: { children: React.ReactNode }) {
  ...
  if (!isHydrated) {
    return <div className="flex items-center justify-center h-64 text-parchment-light/50">{t.common.loading}</div>;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (input.length < 4) { setError(t.master.pin.minLength); return; }
    if (isCreating) {
      await setPin(input);
      setConfirmed(true);
    } else {
      const valid = await verifyPin(input);
      if (valid) { setConfirmed(true); } else { setError(t.master.pin.incorrect); setInput(""); }
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
      ...
      <h2 className="font-cinzel text-2xl text-gold">{isCreating ? t.master.pin.createTitle : t.master.pin.loginTitle}</h2>
      <p className="text-parchment-light/50 text-center max-w-sm">
        {isCreating ? t.master.pin.createDesc : t.master.pin.loginDesc}
      </p>
      <form onSubmit={handleSubmit} className="flex flex-col items-center gap-3">
        ...
        <Button type="submit">{isCreating ? t.master.pin.createBtn : t.master.pin.loginBtn}</Button>
      </form>
    </div>
  );
}
